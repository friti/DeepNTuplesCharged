...
class BareDeepNtuplizer : public edm::one::EDAnalyzer<edm::one::SharedResources>  {
public:
  explicit BareDeepNtuplizer(const edm::ParameterSet&);
  ~BareDeepNtuplizer();
...
BareDeepNtuplizer::BareDeepNtuplizer(const edm::ParameterSet& iConfig):
...
BareDeepNtuplizer::~BareDeepNtuplizer()
{
  return;
  for(auto& m:modules_)
    delete m;
...
void
BareDeepNtuplizer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
...
void
BareDeepNtuplizer::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
DEFINE_FWK_MODULE(BareDeepNtuplizer);
